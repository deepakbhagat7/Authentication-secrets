...
const GoogleStrategy = require('passport-google-oauth20').Strategy; //using Oauth
...
app.get("/auth/google",
  passport.authenticate('google', { scope: ["profile"] })     //retriving info from google
);

app.get("/auth/google/secrets", 
passport.authenticate("google", {failureRedirect: "/login"}),
function (req, res){
    // sucessfully authenticated, redirected to secret page
...
app.post("/submit", function (req, res) {
  ...
  useNewUrlParser.findById(req.user.id, function (err, foundUser) {
      if (err) {
          console.log(err);
      } else {
       if (foundUser) {
